...
import { ArrowLeft, Leaf, Save, Loader2, CheckCircle2, Building2, Lock } from "lucide-react";
import Link from "next/link";

interface Tenant {
  id: string; name: string; slug: string; address: string;
  gstin: string; fssai: string; phone: string; adminPin: string;
}
...
export default function AdminSettings() {
  ...
  const [form, setForm] = useState({ name: "", address: "", gstin: "", fssai: "", phone: "", adminPin: "" });
  ...
  useEffect(() => {
    ...
    try {
      ...
      setForm({ name: t.name, address: t.address ?? "", gstin: t.gstin ?? "", fssai: t.fssai ?? "", phone: t.phone ?? "", adminPin: "" });
  ...
  async function save() {
    ...
    const res = await fetch(`/api/hbl/admin/tenants/${tenant.id}`, {
      ...
      body: JSON.stringify({ name: form.name, address: form.address, gstin: form.gstin, fssai: form.fssai, phone: form.phone }),
    });
