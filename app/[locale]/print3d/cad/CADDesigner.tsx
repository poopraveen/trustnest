...
export default function CADDesigner() {
  ...
  const [tab, setTab] = useState<"design" | "import">("design");
  ...
  const downloadSTL = () => {
    if (tab === "design") {
      ...
      // Re-export from buffer geometry
      const pos = stlGeo.getAttribute("position");
      const nor = stlGeo.getAttribute("normal");
      ...
      for (let i = 0; i < triCount; i++) {
        view.setFloat32(off, nor.getX(i * 3), true); off += 4;
        view.setFloat32(off, nor.getY(i * 3), true); off += 4;
        view.setFloat32(off, nor.getZ(i * 3), true); off += 4;
        for (let v = 0; v < 3; v++) {
          const idx = i * 3 + v;
          view.setFloat32(off, pos.getX(idx), true); off += 4;
          view.setFloat32(off, pos.getY(idx), true); off += 4;
          view.setFloat32(off, pos.getZ(idx), true); off += 4;
        }
        off += 2;
      }
  ...
  // ── Send to order form ───────────────────────────────────────────────────────
  const sendToOrder = () => {
    const s = stats ?? stlStats;
    if (!s) return;
    const title = shapes.length > 0 ? shapes[0].name : "CAD Model";
    router.push(
      ...
      <div className="flex-1 relative" ref={containerRef}>
        ...
        {shapes.length === 0 && !stlGeo && (
          ...
          >
            ...
            <p className="text-sm mt-1">Or import an STL file</p>
          </div>
        )}
      </div>
    </div>
