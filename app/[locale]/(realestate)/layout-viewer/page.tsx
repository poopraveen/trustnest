...
const DURATION  = 750; // ms — matches D3's default transition
...
const SAMPLE_PLOTS: Plot[] = [
  ...
  { id:"P106", name:"Plot 106", status:"sold",      size:"1,350 sqft", price:"₹54 Lakhs", facing:"East",      coordinates:[[270,340],[390,340],[390,440],[270,440]] },
];
...
// ─── Zoom math (mirrors D3 zoom-to-bounding-box) ─────────────────────────────
...
export default function LayoutViewerPage() {
  ...
  // Transform — kept in both ref (for non-reactive handlers) and state (for render)
  const xRef = useRef<XForm>({ tx:0, ty:0, scale:1 });
  const [xform, setXform]   = useState<XForm>({ tx:0, ty:0, scale:1 });
  const [animated, setAnim] = useState(false); // enables CSS transition
  ...
  function resetZoom() {
    applyXform({ tx:0, ty:0, scale:1 }, true);
  }
  ...
  const onBgClick = () => {
    if (mode !== "view") return;
    setSelected(null);
    resetZoom();         // ← D3 pattern: click background → zoom out
  };
  ...
  const onPlotClick = (e: React.MouseEvent, plot: Plot) => {
    ...
    if (selectedId === plot.id) {
      // second click → deselect + zoom out
      setSelected(null);
      resetZoom();
    } else {
      setSelected(plot.id);
      setTab("info");
      zoomToPlot(plot);  // ← D3 pattern: click plot → smooth zoom to bbox
    }
  };
  ...
  function changeStatus(id: string, status: PlotStatus) {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-900 overflow-hidden">
      ...
      {/* ── Toolbar ─────────────────────────────────────────────────────────── */}
      <div className="flex items-center gap-2 px-4 py-2 bg-slate-800 border-b border-slate-700 flex-shrink-0 flex-wrap">
        <span className="font-bold text-white text-sm mr-1">Layout Viewer</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="relative flex-1 overflow-hidden select-none">
          ...
          {!image && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                ...
                <p className="text-slate-500 text-sm">Upload a layout image or interact with the sample plots</p>
              </div>
            </div>
          )}
          ...
          <svg
            ...
            onClick={onSVGClick}
          >
            {/* Background — clicking this resets zoom (D3 pattern) */}
            <rect
            ...
            {/* Transformed group — CSS transition here gives D3-style smooth zoom */}
            <g style={gStyle}>
              ...
              {plots.map(plot => {
                ...
                return (
                  <g key={plot.id}>
                    ...
                    {/* Vertex handles (edit-vertex mode + selected) */}
            ...
            {tab==="upload" && (
              <div className="p-4 space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500">PNG, JPG or SVG accepted. Used as background for drawing plot boundaries.</p>
                </div>
                <div
                ...
                {image && (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => { setImage(null); resetZoom(); }} className="text-xs py-1.5 px-3 bg-red-50 text-red-600 font-semibold rounded-lg border border-red-200 hover:bg-red-100 transition-colors">Remove</button>
                    </div>
                  </div>
                )}
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-3">
                  ...
                  <ul className="text-xs text-blue-700 space-y-1 list-disc list-inside">
                    ...
                    <li>Export JSON saves all coordinates</li>
                  </ul>
                </div>
              </div>
            )}
      ...
      {/* Dash animation for selection ring */}
