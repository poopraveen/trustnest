...
export async function GET(req: NextRequest) {
  ...
  const [orders, popularProducts, retentionData, newMembers] = await Promise.all([
    prisma.hblOrder.findMany({
      where: { ...tw, createdAt: { gte: startDate } },
      include: { items: { include: { product: { select: { name: true, category: true } } } } },
      orderBy: { createdAt: "asc" },
    }),
    prisma.hblOrderItem.groupBy({
      by: ["productId"],
      where: { order: { ...tw, createdAt: { gte: startDate } } },
      _sum: { qty: true },
      orderBy: { _sum: { qty: "desc" } },
      take: 5,
    }),
    prisma.hblMember.groupBy({ by: ["status"], where: tw, _count: { id: true } }),
    prisma.hblMember.count({ where: { ...tw, joinedAt: { gte: startDate } } }),
  ]);
  ...
  const popular = popularProducts.map((p) => ({ ...p, product: productDetails.find((d) => d.id === p.productId) }));
